...
import { Filter, CalendarCheck, PlusCircle, CalendarX, Plus, Edit2, Trash2, CheckCircle2 } from 'lucide-react';

const AdminTable = ({ 
  adminSection, currentSection, activeSeasonId, setActiveSeasonId, seizoenen, 
  zaalTab, setZaalTab, setEditingItem, setUitzonderingType, setShowAdminModal, 
  setShowBulkScheduleModal, setSelectedCoachIds, setTempVasteTraining, 
  openEditModal, handleDeleteVasteTraining, trainingen, groepen, coaches, locaties, beschikbareZalen,
  db, deleteDoc, doc
}) => {
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          {adminSection === 'beschikbareZalen' && zaalTab === 'uitzonderingen' ? (
            ...
            <button onClick={() => { setEditingItem(null); setSelectedCoachIds([]); setTempVasteTraining({dag:'', startUur:'', eindUur:''}); setShowAdminModal(true); }} className="bg-slate-900 text-white px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-indigo-600 transition text-sm font-bold"><Plus size={16}/> Toevoegen</button>
          )}
        </div>
      </div>
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition">
                      ...
                      <button onClick={() => adminSection === 'vasteTrainingen'? handleDeleteVasteTraining(item, trainingen, isIngepland) : (window.confirm("Verwijderen?") && deleteDoc(doc(db, currentSection.collection, item.id)))} className="p-1.5 text-slate-400 hover:text-red-600"><Trash2 size={14}/></button>
                    </div>
                  </td>
                </tr>
              ))
            ) : (
              <tr><td colSpan="100%" className="px-4 py-12 text-center text-slate-400 text-sm">Geen gegevens gevonden voor dit seizoen.</td></tr>
            )}
          </tbody>
        </table>
      </div>
